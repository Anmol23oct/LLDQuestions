...
class Record:
    ...
    def set_field(self, field: str, value: str, timestamp: int | None = None, ttl: int | None = None):
        expires_at = timestamp + ttl if ttl is not None and timestamp is not None else None
    ...
    def clone_with_remaining_ttl(self, at_time: int) -> 'Record' | None:
        new_record = Record()
        for field, field_obj in self.fields.items():
            if not field_obj.is_valid(at_time):
                continue
            ttl_remaining = (
                field_obj.expires_at - at_time
                if field_obj.expires_at is not None
                else None
            )
            new_record.set_field(field, field_obj.value, 0, ttl_remaining)
        return new_record if new_record.fields else None
