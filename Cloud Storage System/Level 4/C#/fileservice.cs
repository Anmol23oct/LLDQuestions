...
public class FileService
{
    ...
    public string MergeUser(string user1Id, string user2Id)
    {
        ...
        // Clean up files owned by user2
        var filesToDelete = _fileRepo.GetAll()
            .Where(x => x.Item2 == user2Id)
            .Select(x => x.Item1.Path)
            .ToList();

        foreach (var path in filesToDelete)
        {
            _fileRepo.Delete(path);
        }
    ...
    public string BackupUser(string userId)
    {
        ...
        var fileSizes = _fileRepo.GetAll()
            .Where(e => e.Item2 == userId)
            .ToDictionary(e => e.Item1.Path, e => e.Item1.Size);

        return user.Backup(fileSizes).ToString();
    }
    ...
    public string RestoreUser(string userId)
    {
        ...
        int restored = user.Restore(
            path => _fileRepo.GetOwner(path),
            (path, size) => _fileRepo.Add(new File(path, size), userId)
        );
