...
class FileService:
    ...
    def merge_user(self, user1_id: str, user2_id: str) -> str:
        ...
        if result is None:
            return ""
        # cleanup files of user2
        for file, owner in self.file_repo.get_all():
            if owner == user2_id:
                self.file_repo.delete(file.path)
        return str(result)
    ...
    def backup_user(self, user_id: str) -> str:
        ...
        return str(user.backup(owned_files))
    ...
    def restore_user(self, user_id: str) -> str:
        ...
        def get_owner(path):
            return self.file_repo.get_owner(path)

        def restore_file(path, size):
            file = File(path, size)
            self.file_repo.add(file, user_id)

        return str(user.restore(get_owner, restore_file))
